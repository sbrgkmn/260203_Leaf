...
export function atlasTable(rows) {
  ...
  const headings = Array.from({length:count},(_,i) => `<th scope="col"><b>${i%2?'C':'E'}</b><span>${String(i+1).padStart(2,'0')}</span></th>`).join('');
  return `<table class="atlas-table"><caption>All generated E/C operations, starting at 01 / E. Each row uses a fixed scale. Green dividers begin blades; ochre dividers begin margins.</caption><thead><tr><th scope="col" class="row-name">Leaf / Figure 7</th>${headings}<th scope="col" class="atlas-final">Final</th></tr></thead><tbody>${rows.map(row => {
...
export function atlasSvg(rows) {
  ...
  let content=text(18,28,'Metamorphic leaves / 16 development sequences',22)+text(18,51,'Paper-inspired parametric studies. Every E/C operation shown; shared scale within each row.');
  for(let i=0;i<count;i++)content+=text(label+i*cell+45,80,`${String(i+1).padStart(2,'0')} / ${i%2?'C':'E'}`);
  ...
  rows.forEach(row=>{
    ...
    row.steps.forEach((step,i)=>{
      ...
      if(step.stage>1&&(i===0||row.steps[i-1].stage!==step.stage))content+=`<path d="M ${label+i*cell} ${y} v ${rowHeight}" stroke="${step.stage===3?'#a98339':'#586b43'}" stroke-width="2"/>`;
    });
